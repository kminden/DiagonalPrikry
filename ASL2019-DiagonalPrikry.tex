...
%\begin{definition} Let $\M$ be a transitive structure with potentially infinitely many predicates. A theory is $\Sigma_1(\M)$, also known as \emph{$\M$-recursively enumerable} or $\M$-$re$, if the theory is $\Sigma_1$-definable, with parameters from $\M$. \end{definition}
...
\begin{frame} %\frametitle{Transferring Consistency via Cofinal Embeddings}
