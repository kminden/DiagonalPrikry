...
\subjclass[2010]{03E40, 03E55}
\keywords{subcomplete forcing, generalized Prikry forcing, iterated forcing, large cardinals}
...
Subcomplete forcing notions are a family of forcing notion that do not add reals and may be iterated using revised countable support. Examples of subcomplete forcing include all countably closed forcing, Prikry forcing, and Namba forcing (under $\CH$), as shown by Jensen~\cite[Section 3.3]{Jensen:2014}. It is clear from these examples that not all subcomplete forcing notions are proper, and, conversely, nontrivial $ccc$ forcing notions are never subcomplete \cite{Minden:2017fr}. For more on subcomplete forcing and its characteristics, refer to Jensen (primarily \cite{Jensen:2014} and \cite{Jensen:2009fe}), and the author's doctoral thesis \cite{Minden:2017fr}.
...
%\begin{equation} \tag{$\star$} \label{eqn:SkDotSigma=SkTildeSigma} \sk{\widetilde N}{\widetilde \delta}{\widetilde \sigma'} = \sk{\widetilde N}{\widetilde \delta}{\widetilde \sigma}. \end{equation}
%%let $x \in \sk{N}{\delta}{\sigma'}$. Then we have that $N$ sees that there are some formula $\varphi$, $\overline z \in \N$, and $\xi < \delta$ so that $x$ uniquely satisfies $\varphi(x, \pi(\widetilde \sigma'(\overline z)), \xi)$. In particular, $x$ is in the range of $\pi$. 
%%%%%M Why is this?
%%Thus $\widetilde x = \pi^{-1}(x) \in \widetilde N$ and there is some $\widetilde \xi < \widetilde \delta$ such that $\varphi(\widetilde x, \widetilde \sigma'(\overline z), \widetilde \xi)$ holds. Thus by (\ref{eqn:SkDotSigma=SkTildeSigma}), we have that $\widetilde x$ is unique such that 
%%$\varphi(\widetilde x, \widetilde \sigma(\overline y), \widetilde \zeta)$ for some $\overline y \in \N$ and $\widetilde \zeta < \widetilde \delta$. Thus pushing back up through $\pi$, letting $\zeta = \pi(\widetilde \zeta)$, we have that $x = \pi(\widetilde x)$ is unique such that $\varphi(x, \sigma(\overline y), \zeta)$ holds, so $x \in \Sk{N}{\delta}{X}$.
%%
%Showing $\Sk{N}{\delta}{X} \subseteq \sk{N}{\delta}{\sigma'}$ works similarly. Again by (\ref{eqn:SkDotSigma=SkTildeSigma}) we have $\ran{\widetilde \sigma} \subseteq \sk{\widetilde N}{\widetilde \delta}{\widetilde \sigma'}$.
...
%%So we may find $\widetilde \xi < \widetilde \delta$ such that $\widetilde x = \pi^{-1}(x)$ is unique such that $\varphi(\widetilde x, \widetilde \sigma(\overline z), \widetilde \xi)$. So by (\ref{eqn:SkDotSigma=SkTildeSigma}), we have that there is $\overline y \in \N$ and $\widetilde \zeta < \widetilde \delta$ such that $\widetilde x$ is unique satisfying $\varphi(\widetilde x, \dot{\sigma}^{\mathfrak A}(\overline y), \widetilde \zeta)$. Finally, by pushing back up through $\pi$, letting $\pi(\widetilde \zeta) = \zeta$, we have that $x = \pi(\widetilde x)$ is unique such that $\varphi(x, \sigma'(\overline y), \zeta)$, so $x \in \sk{N}{\delta}{\sigma'}$ as desired.

%\begin{eqnarray*}
%	x \in \sk{N}{\delta}{\sigma'} 
%	&\iff& N \models \left( x = \text{that } y \text{ such that } \varphi(y, \pi(\widetilde \sigma'(\overline z)), \xi) \right) \text{ where } \overline z \in \overline N, \xi<\delta \\
%	&\iff& \widetilde N \models \left( \pi^{-1}(x) = \text{that } y \text{ such that } \varphi(y, \widetilde \sigma'(\overline z), \widetilde \xi) \right) \text{ where } \overline z \in \overline N, \widetilde \xi< \widetilde \delta \\
%	&\iff& \widetilde N \models \left( \pi^{-1}(x) = \text{that } y \text{ such that } \varphi(y, \widetilde \sigma(\overline z), \widetilde \xi) \right) \text{ where } \overline z \in \overline N, \widetilde \xi<\widetilde \delta\\
